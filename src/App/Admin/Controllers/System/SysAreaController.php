...
class SysAreaController extends AdminController
{
    public $model = \Shopwwi\Admin\App\Admin\Models\SysArea::class;
    public  $orderBy = ['id' => 'asc'];
    protected $adminOp = true;

    protected $trans = 'sysArea'; // 语言文件名称
    protected $queryPath = 'system/area'; // 完整路由地址
    protected $activeKey = 'settingSiteArea';
    /**
     * 路由注册
     * @var string
     */
     public $routePath = 'area'; // 当前路由模块不填写则直接控制器名
    ...
    protected function fields(){
        $deep = [['label'=>'省级','value'=>0],['label'=>'市级','value'=>1],['label'=>'区/县级','value'=>2],['label'=>'街道','value'=>3]];
        $deepOne = [0=>'省级',1=>'市级',2=>'区/县级',3=>'街道','*'=>'${deep}'];
        return [
            shopwwiAmisFields(trans('field.id',[],'sysArea'),'id')->showOnCreation(false)->showOnUpdate(false)->filterColumn('input-text'),
            shopwwiAmisFields(trans('field.name',[],'sysArea'),'name')->rules('required'),
            shopwwiAmisFields(trans('field.pid',[],'sysArea'),'pid')->rules('required'),
            shopwwiAmisFields(trans('field.deep',[],'sysArea'),'deep')
                ->rules('required')
                ->tableColumn(['sortable'=>true,'type'=>'mapping','map'=>$deepOne])
                ->filterColumn('select',['options'=>$deep])
                ->column('select',['options'=>$deep]),
            shopwwiAmisFields(trans('field.lng',[],'sysArea'),'lng'),
            shopwwiAmisFields(trans('field.lat',[],'sysArea'),'lat'),
            shopwwiAmisFields(trans('field.code',[],'sysArea'),'code'),
            shopwwiAmisFields(trans('field.initial',[],'sysArea'),'initial'),
            shopwwiAmisFields(trans('field.pinyin',[],'sysArea'),'pinyin'),
            shopwwiAmisFields(trans('field.ext_name',[],'sysArea'),'ext_name')->rules('required'),
