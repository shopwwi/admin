...
class SysHelpClassController extends AdminController
{
    protected $model = \Shopwwi\Admin\App\Admin\Models\SysHelpClass::class;
    protected $orderBy = ['id' => 'desc'];
    protected $adminOp = true;

    protected $trans = 'sysHelpClass'; // 语言文件名称
    protected $queryPath = 'system/help/class'; // 完整路由地址
    protected $activeKey = 'settingContentHelpClass';
    /**
     * 路由注册
     * @var string
     */
     public $routePath = 'help/class'; // 当前路由模块不填写则直接控制器名
    ...
    protected function fields(){
        $sysHelpPosition = DictTypeService::getAmisDictType('sysHelpPosition');
        return [
            ...
            shopwwiAmisFields(trans('field.name',[],'sysHelpClass'),'name')->rules(['bail','required','min:1','max:30']),
            shopwwiAmisFields(trans('field.position',[],'sysHelpClass'),'position')->tableColumn(['sortable'=>true,'type'=>'mapping','map'=>$this->toMappingSelect($sysHelpPosition,'${position}','default')])
                ->filterColumn('select',['options'=>$sysHelpPosition])
                ->column('select',['options'=>$sysHelpPosition])->rules(['bail','required','numeric','min:0']),
