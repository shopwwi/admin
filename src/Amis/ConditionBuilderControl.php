 ...
 * @method self type($value) 指定为
 * @method self pickerIcon($value) 非内嵌模式时 弹窗触发icon
 ...
 * @method self builderMode($value) 展现模式 可选值: simple | full | 
 * @method self staticOn($value) 是否静态展示表达式
 * @method self fields($value) 字段集合
 ...
 * @method self showANDOR($value) 是否显示并或切换键按钮，只在简单模式下有用
 * @method self disabled($value) 是否禁用
 * @method self embed($value) 内嵌模式，默认为 true
