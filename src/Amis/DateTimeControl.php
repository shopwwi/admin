 ...
 * @method self inputFormat($value) 日期展示格式
 ...
 * @method self timeFormat($value) 时间的格式。
 ...
 * @method self format($value) 日期存储格式
 * @method self timeConstraints($value) 时间输入范围限制
 * @method self staticInputClassName($value) 静态展示表单项Value类名
 * @method self name($value) 字段名，表单提交时的 key，支持多层级，用.连接，如： a.b.c
 * @method self utc($value) 设定是否存储 utc 时间。
 ...
 * @method self emebed($value) 是否为内联模式？
 * @method self width($value) 在Table中调整宽度
 * @method self disabledOn($value) 是否禁用表达式
 * @method self id($value) 组件唯一 id，主要用于日志采集
 * @method self staticLabelClassName($value) 静态展示表单项Label类名
 * @method self type($value) 指定为日期时间选择控件
