 ...
 * @method self addOn($value) 
 * @method self showCounter($value) 是否显示计数
 ...
 * @method self transform($value) 自动转换值
 ...
 * @method self maxLength($value) 限制文字最大输入个数
 * @method self nativeInputClassName($value) 原生input标签的CSS类名
 ...
 * @method self minLength($value) 限制文字最小输入个数
 * @method self clearValueOnEmpty($value) 在内容为空的时候清除值
 ...
 * @method self inputControlClassName($value) control节点的CSS类名
 ...
 * @method self type($value) 表单项类型 可选值: input-text | input-email | input-url | input-password | native-date | native-time | native-number | 
