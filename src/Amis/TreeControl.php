 ...
 * @method self withChildren($value) 选父级的时候是否把子节点的值也包含在内。
 * @method self showOutline($value) 是否显示展开线
 ...
 * @method self highlightTxt($value) 需要高亮的字符串
 ...
 * @method self rootValue($value) 顶级选项的值
 * @method self enableNodePath($value) 是否开启节点路径模式
 * @method self pathSeparator($value) 开启节点路径模式后，节点路径的分隔符
 * @method self clearable($value) 是否可清除。
 * @method self cascade($value) 该属性代表数据级联关系，autoCheckChildren为true时生效，默认为false，具体数据级联关系如下： 1.casacde为false，ui行为为级联选中子节点，子节点禁用；值只包含父节点的值 2.cascade为false，withChildren为true，ui行为为级联选中子节点，子节点禁用；值包含父子节点的值 3.cascade为true，ui行为级联选中子节点，子节点可反选，值包含父子节点的值，此时withChildren属性失效 4.cascade不论为true还是false，onlyChildren为true，ui行为级联选中子节点，子节点可反选，值只包含子节点的值
 ...
 * @method self rootCreatable($value) 顶级节点是否可以创建子节点
