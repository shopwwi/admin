 ...
 * @method self marks($value) 刻度
 * @method self id($value) 组件唯一 id，主要用于日志采集
 * @method self staticClassName($value) 静态展示表单项类名
 * @method self showSteps($value) 是否展示步长
 ...
 * @method self showInput($value) 是否展示输入框
 * @method self onEvent($value) 事件动作配置
 * @method self staticOn($value) 是否静态展示表达式
 * @method self tooltipPlacement($value) 标签方向
 ...
 * @method self parts($value) 分割块数
 * @method self multiple($value) 是否为双滑块
 * @method self description($value) 描述内容，支持 Html 片段。
 * @method self clearable($value) 输入框是否可清除
 * @method self disabledOn($value) 是否禁用表达式
 * @method self staticPlaceholder($value) 静态展示空值占位
 * @method self max($value) 最大值
 * @method self unit($value) 单位
 * @method self joinValues($value) 是否通过分隔符连接
 ...
 * @method self value($value) 滑块值
 * @method self tooltipVisible($value) 是否展示标签
