 ...
 * @method self advancedSettings($value) 自定义详情配置面板如：{   boolean: [      {type: "input-text", name: "aa", label: "AA" }   ] }当配置布尔字段详情时，就会出现以上配置
 * @method self clearValueOnHidden($value) 表单项隐藏时，是否在当前 Form 中删除掉该表单项值。注意同名的未隐藏的表单项值也会删掉
 * @method self rootTypeMutable($value) 顶层是否允许修改类型
 ...
 * @method self type($value) 指定为 JSON Schema Editor
 * @method self disabledTypes($value) 禁用类型，默认禁用了 null 类型
 ...
 * @method self showRootInfo($value) 顶层类型信息是否隐藏
 ...
 * @method self definitions($value) 可以理解为类型模板，方便快速定义复杂类型
 * @method self enableAdvancedSetting($value) 开启详情配置
