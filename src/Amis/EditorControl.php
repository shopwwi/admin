 ...
 * @method self editorDidMount($value) 获取编辑器底层实例
 ...
 * @method self language($value) 语言类型 可选值: bat | c | coffeescript | cpp | csharp | css | dockerfile | fsharp | go | handlebars | html | ini | java | javascript | json | less | lua | markdown | msdax | objective-c | php | plaintext | postiats | powershell | pug | python | r | razor | ruby | sb | scss | shell | sol | sql | swift | typescript | vb | xml | yaml | 
 * @method self labelAlign($value) 描述标题
 * @method self descriptionClassName($value) 配置描述上的 className
 * @method self id($value) 组件唯一 id，主要用于日志采集
 * @method self size($value) 编辑器大小 可选值: sm | md | lg | xl | xxl | 
 */
