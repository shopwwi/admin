...
<body>
    @php
        ...
        switch ($code){
            case 401:
                $msg = '请先完成登入后在操作';
                $url = [['url'=>shopwwiUserUrl('auth/login'),'msg'=>'立即登入'],['url'=>'javascript:history.back()','msg'=>'返回上一页']];
                break;
            case 402:
                $msg = '登入超时，请重新登入';
                $url = [['url'=>shopwwiUserUrl('auth/login'),'msg'=>'重新登入'],['url'=>'javascript:history.back()','msg'=>'返回上一页']];
                break;
            case 403:
                $msg = '无权限访问，请返回重试';
                $url = '';
